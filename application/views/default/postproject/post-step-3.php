...
<div id="dataStep-3" style="display: none1">
    ...
    <div class="content with-padding">
      <div class="submit-field myradio mb-0">
        ...
        <div class="btn-group">
          ...
          <label for="defaultInline<?php D($key);?>" class="btn <?php if($projectData && $projectData['project_settings']->project_type_code==$key){echo 'active';}?>"><i class="icon-material-<?php D($key);?>"></i><br>
            <?php D($keydata['name']);?>
          </label>
          <?php
                            }
        ...
        <span id="projectTypeError" class="rerror"></span> </div>
    </div>
  </div>
  <div class="dashboard-box"> 
    <!-- Headline -->
      ...
      <div class="submit-field mb-0">
        ...
        <div id="addQuestion_container">
          ...
          <div class="question_sec input-group margin-bottom-10">
            ...
            <div class="input-group-append"> <a href="<?php D(VZ);?>" class="btn text-danger" onclick="$(this).closest('.question_sec').remove()"><i class="icon-feather-x f20"></i></a> </div>
          </div>
          <?php
                		}
                	}?>
        </div>
        <span id="questionError" class="rerror"></span> <a href="javascript:void(0)" class="btn btn-outline-success" id="addQuestion"><?php echo __('postproject_add','+ Add');?> </a> </div>
