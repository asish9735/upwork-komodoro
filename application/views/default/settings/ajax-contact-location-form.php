...
<form action="" method="post" accept-charset="utf-8" id="locationform" class="form-horizontal" role="form" name="locationform" onsubmit="updateLocation();return false;">  
	<div class="row">
			...
			<select name="city_id" id="city_id" data-size="4" class=" selectpicker" title="Select city" data-live-search="true">
				...
				if($city){
					foreach($city as $city_list){
						?>
						<option value="<?php echo $city_list->city_id;?>" <?php if($city_list->city_id==$memberInfo->city_id){echo 'selected';}?>><?php echo ucfirst($city_list->city_name);?></option>
						<?php
					}
				}
					?>
		...
		<span id="mobile_codeError" class="rerror"></span>			
			...
			<a href="javascript:void(0)" class="btn btn-secondary me-2" id="cancel_location"><?php echo __('setting_contact_info_cancel','Cancel');?> </a>
			<button class="btn btn-site locationUpdateBTN"><?php echo __('setting_contact_info_update','Update');?></button>
</form>
