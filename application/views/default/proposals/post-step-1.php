...
<div id="dataStep-1" style="display: nones"> 
  ...
  <div class="dashboard-box"> 
    <!-- Headline -->
    <div class="headline">
      <h4> <?php echo __('postproposal_title','Title');?> </h4>
    </div>
    <div class="content with-padding">
      <label class="form-label"><?php echo __('postproposal_proposal_name','Name of your service');?></label>
  ...
  <div class="dashboard-box"> 
    <!-- Headline -->
    <div class="headline">
      <h4><?php echo __('postproposal_category','Category');?></h4>
    </div>
    <div class="content with-padding">
          ...
          <?php
                                if($all_category){
                                    foreach($all_category as $category_list){
                                        ?>
          <option value="<?php D($category_list->category_id);?>" <?php if($proposalData && $proposalData['proposal_category']->category_id==$category_list->category_id){echo 'selected';}?>>
              ...
              <?php
                                if($all_sub_category){
                                    foreach($all_sub_category as $sub_category_list){
                                        ?>
              <option value="<?php D($sub_category_list->category_subchild_id);?>" <?php if($proposalData && $proposalData['proposal_category']->category_subchild_id==$sub_category_list->category_subchild_id){echo 'selected';}?>>
  ...
  <div class="dashboard-box">
    <div class="headline">
      <h4><?php echo __('postproposal_tags','Tags');?> </h4>
    </div>
    <div class="content with-padding">
      <div class="submit-field">
        <label class="form-label"><?php echo __('postproposal_required_tags','Select tags');?></label>
        <div class="keywords-list skillContaintag"></div>
        <input  class="form-control tagsinput_skill" name="skills" id="skills" value="">
        <span id="skillsError" class="rerror"></span> </div>      
    </div>
  </div>
